 ...
 <header role="banner" style="background-color: black">
    ...
    <section class="site-section bg-light">
      <div class="container">
        ...
        @foreach($total_booking_room->chunk(3) as $room )
        <div class="row ">

            @foreach($room as $booking_room)
           
             @if(($booking_room->number_of_room-$booking_room->booking_room) > 0)
              <div class="col-md-4">
                <div class="post-entry">
                  <img src="{{ asset($booking_room->image) }}" alt="Image placeholder" class="img-fluid">
                  <div class="body-text">
                    <div class="category">{{$booking_room->name}}</div>
                    <h3 class="mb-3"><a href="{{url('/room-details/'.$booking_room->id)}}">{{$booking_room->name}}(Available room {{$booking_room->number_of_room-$booking_room->booking_room}})</a></h3>
                    <p class="mb-4">{{$booking_room->details}}</p>
                    <p class="mb-4" style="font-style: oblique; font-weight: 900;">Price :{{$booking_room->price}} BDT</p>
                    <p><a href="{{url('/room-details/'.$booking_room->id)}}" class="btn btn-primary btn-outline-primary btn-sm">View Details</a></p>
                  </div>
                </div>
              </div>
              @endif
        ...
        @foreach($hotel_room->chunk(3) as $rooms )
        <div class="row ">
              ...
              <div class="col-md-4">
                <div class="post-entry">
                  ...
                  <div class="body-text">
                    ...
                    <h3 class="mb-3"><a href="{{url('/room-details/'.$room->id)}}"></a></h3>
                    <p class="mb-4">{{$room->details}}</p>
                    <p class="mb-4" style="font-style: oblique; font-weight: 900;">Price :{{$room->price}} BDT</p>
                    <p><a href="{{url('/room-details/'.$room->id)}}" class="btn btn-primary btn-outline-primary btn-sm">View Details</a></p>
                  </div>
                </div>
              </div>
